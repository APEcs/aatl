## @file
# This file contains the implementation of the AATL tag handling engine.
#
...
# @param persist    Optional cross-course persistence flag. If true, this tag assignment
#                   will be copied across courses.
...
sub attach {
    ...
    my $newh = $self -> {"dbh"} -> prepare("INSERT INTO ".$self -> {"settings"} -> {"database"} -> {"metadata_tags"}."
                                            (metadata_id, tag_id, attached_by, attached_date, rating, persist)
                                            VALUES(?, ?, ?, UNIX_TIMESTAMP(), ?, ?)");
    my $rows = $newh -> execute($metadataid, $tagid, $userid, $rating, $persist);
...
## @method $ is_active($tagid, $courseid, $only_course)
# Is the tag specified active within the course (or the whole site)? This does not
# actually care whether the tag is actually *attached* to anything, simply whether it
# would be considered active if it was.
#
# @param tagid       The ID of the tag to check the activation state of.
# @param courseid    The ID of the course to check activation within, or undef to check
#                    site-wide activation.
# @param only_course If true, this checks only the course level (and requires courseid
#                    to be set). Otherwise it will check course and site level activations.
# @return true if the tag is active, false if it is not, undef on error.
sub is_active {
    my $self        = shift;
    my $tagid       = shift;
    my $courseid    = shift;
    my $only_course = shift;

    # Craft a query base on whether the courseid is set...
    my $query = "SELECT id FROM ".$self -> {"settings"} -> {"database"} -> {"active_tags"}."
                 WHERE tag_id = ?
                 AND (";
    $query .= "course_id is NULL" unless($only_course);
    $query .= " OR " if($courseid && !$only_course);
    $query .= "course_id = ?" if($courseid);
    $query .= ")";
    my $checkh = $self -> {"dbh"} -> prepare($query);

    # and run the query based on courseid availability
    my $result;
    if($courseid) {
        $result = $checkh -> execute($tagid, $courseid);
    } else {
        $result = $checkh -> execute($tagid);
    }
    return $self -> self_error("Unable to perform tag activation check: ". $self -> {"dbh"} -> errstr) if(!$result);
...
## @method $ activate($tagid, $userid, $courseid)
# Flag a tag as active within the context of the specified course (or across the whole
# system, if the courseid is not specified). Note that this does not attempt to check
# whether the user has permission to activate the tag, the caller is required to
# determine whether the user has permission.
#
# @param tagid    The ID of the tag to activate.
# @param userid   The ID of the user activating the tag.
# @param courseid Optional course ID. If specified, the tag is only considered active
#                 within the specified course, otherwise it is active across the whole
#                 system.
# @return true on success (even if the tag is already active), undef on error.
sub activate {
    my $self     = shift;
    my $tagid    = shift;
    my $userid   = shift;
    ...
    my $active = $self -> is_active($tagid, $courseid);
    if($active) {
        $self -> {"logger"} -> log("warning", $userid, undef, "Attempt to activate already active tag $tagid by $userid.");
        return 1;
    }

    # Tag isn't active, make it so
    my $acth = $self -> {"dbh"} -> prepare("INSERT INTO ".$self -> {"settings"} -> {"database"} -> {"active_tags"}."
                                            (course_id, tag_id, user_id)
                                            VALUES(?, ?, ?)");
    my $rows = $acth -> execute($courseid, $tagid, $userid);
    return $self -> self_error("Unable to perform tag activate: ". $self -> {"dbh"} -> errstr) if(!$rows);
    return $self -> self_error("Tag activation failed: no rows modified") if($rows eq "0E0");

    return 1;
}


## @method $ deactivate($tagid, $courseid)
# Remove the activated status of a tag in a specified course (or across the whole
# system, if the courseid is not specified). Note that this does not attempt to check
# whether the user has permission to deactivate the tag, the caller is required to
# determine whether the user has permission.
#
# @param tagid    The ID of the tag to deactivate.
# @param courseid Optional course ID. If specified, any activation marker for the tag
#                 is removed from this course, but note that if the tag has been
#                 activated at the site level as well, it will still appear to be
#                 active within the course!
# @return true on success (even if the tag is already inactive), undef on error.
sub deactivate {
    my $self     = shift;
    my $tagid    = shift;
    my $courseid = shift;

    my $nukeh = $self -> {"dbh"} -> prepare("DELETE FROM ".$self -> {"settings"} -> {"database"} -> {"active_tags"}."
                                             WHERE tag_id = ?
                                             AND course_id ".($courseid ? "= ?" : "IS NULL"));
    my $rows;
    if($courseid) {
        $rows = $nukeh -> execute($tagid, $courseid);
    } else {
        $rows = $nukeh -> execute($tagid);
    }
    return $self -> self_error("Unable to perform tag deactivation: ". $self -> {"dbh"} -> errstr) if(!$rows);
    return $self -> self_error("Tag deactivation failed: no rows modified (tag not active?)") if($rows eq "0E0");
