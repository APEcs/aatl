## @file
# This file contains the implementation of the AATL Role handling engine.
#
...
## @method $ user_assign_role($metadataid, $userid, $roleid, $sourceid, $persist)
...
#       may have Role A set by Source A with no persist, and Role A set by Source B
#       with persistence. In practice this feature is unlikely to be needed or used,
...
# @param persist    If true, the role assignment persists over course copies.
# @return true on success, false if an error occurred.
sub user_assign_role {
    my $self = shift;
    my ($metadataid, $userid, $roleid, $sourceid, $persist) = @_;

    $self -> clear_error();

    # Try to get the role, bomb if an error occurred
    my $role = $self -> _fetch_user_role($metadataid, $userid, $roleid, $sourceid);
    ...
    if(!$role) {
        my $newh = $self -> {"dbh"} -> prepare("INSERT INTO ".$self -> {"settings"} -> {"database"} -> {"metadata_roles"}."
                                                (metadata_id, role_id, user_id, source_id, attached, touched, persist)
                                                VALUES(?, ?, ?, ?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP(), ?)");
        $rows = $newh -> execute($metadataid, $roleid, $userid, $sourceid, $persist);
    ...
    # update its touched timestamp, and potentially change the persist flag
    } else {
        my $oldh = $self -> {"dbh"} -> prepare("UPDATE ".$self -> {"settings"} -> {"database"} -> {"metadata_roles"}."
                                                SET touched = UNIX_TIMESTAMP(),
                                                    persist = ?
                                                WHERE id = ?");
        $rows = $oldh -> execute($persist, $role -> {"id"});
...
## @method $ user_remove_role($metadataid, $userid, $roleid, $sourceid)
...
sub user_remove_role {
    my $self = shift;
    my ($metadataid, $userid, $roleid, $sourceid) = @_;
    ...
    # I really wish execute() would let you supply additional bind params and ignore any that aren't used...
    # As it is, execute differently depending on sourceid setting...
    my $rows;
    if($sourceid) {
        # Delete only allocations of the role made by this source
        $rows = $nukeh -> execute($metadataid, $roleid, $userid, $sourceid);
    } else {
        # Delete all allocations of the role, source be damned.
        $rows = $nukeh -> execute($metadataid, $roleid, $userid);
    }
...
# will pull the role capavility settings out of the database and return a hash
...
## @method private $ _fetch_user_role($metadataid, $userid, $roleid, $sourceid)
# Obtain the metadata role record for the user and role. If sourceid is specified,
# this will only return a metadata role record if it was granted by the source.
#
...
sub _fetch_user_role {
    ...
    my $roleh = $self -> {"dbh"} -> prepare("SELECT *
                                             FROM ".$self -> {"settings"} -> {"database"} -> {"metadata_roles"}."
                                             WHERE metadata_id = ?
                                             AND user_id = ?
                                             AND role_id = ?");
    $roleh -> execute($metadataid, $userid, $roleid)
        or return $self -> self_error("Unable to perform metadata role lookup: ". $self -> {"dbh"} -> errstr);

    # The above query may return multiple rows. Hence if must be looped over...
    while(my $role = $roleh -> fetchrow_hashref()) {
        # skip entries that do not match the source, if needed
        next if($sourceid && $role -> {"source_id"} != $sourceid);

        # Get here, and we have a row, and either the source id matches or
        # hasn't been set, so the row is usable
        $roleh -> finish(); # no need for any more results now
        return $role;
    }
