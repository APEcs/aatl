...
## @class
#
package AATL::QAForums;

use strict;
use base qw(AATL);
use Webperl::Utils qw(is_defined_numeric);
use AATL::System::QAForums;
use POSIX qw(ceil);
use HTML::Scrubber;
...
# @return A reference to a new AATL::QAForums object on success, undef on error.
sub new {
    my $invocant = shift;
    ...
    $self -> {"qaforums"} = AATL::System::QAForums -> new(dbh      => $self -> {"dbh"},
                                                          settings => $self -> {"settings"},
                                                          logger   => $self -> {"logger"},
                                                          roles    => $self -> {"system"} -> {"roles"},
                                                          metadata => $self -> {"system"} -> {"metadata"},
                                                          courses  => $self -> {"system"} -> {"courses"})
        or return Webperl::SystemModule::set_error("Forum initialisation failed: ".$AATL::System::QAForums::errstr);
...
sub _show_question_list {
    ...
    my $pagesize = ($self -> {"settings"} -> {"config"} -> {"AATL::QAForums::question_count"} || 15);
    ...
    foreach my $question (@{$questions}) {
        ...
        $preview = $self -> {"template"} -> truncate_words($preview, $self -> {"settings"} -> {"config"} -> {"AATL::QAForums::preview_length"});
