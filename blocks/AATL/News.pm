...
# @return A reference to a new AATL::News object on success, undef on error.
sub new {
    my $invocant = shift;
    ...
    $self -> {"news"} = AATL::System::News -> new(dbh      => $self -> {"dbh"},
                                                  settings => $self -> {"settings"},
                                                  logger   => $self -> {"logger"},
                                                  roles    => $self -> {"system"} -> {"roles"},
                                                  metadata => $self -> {"system"} -> {"metadata"},
                                                  courses  => $self -> {"system"} -> {"courses"})
        or return Webperl::SystemModule::set_error("News initialisation failed: ".$System::News::errstr);
...
sub build_newpost_form {
    ...
    $canpost = $canpost && !defined($self -> {"cgi"} -> param("postid"))
        unless($self -> {"settings"} -> {"config"} -> {"AATL::News::always_post"});
...
#                the AATL::News::post_count setting variable is used instead.
...
sub build_post_list {
    my $self           = shift;
    my $offset         = shift || 0;
    my $count          = shift || $self -> {"settings"} -> {"config"} -> {"AATL::News::post_count"};
...
# postid as the first post to return, and it will return up to AATL::News::post_count
...
sub build_api_more_response {
    ...
    eval { $posts = $self -> build_post_list($offset, $self -> {"settings"} -> {"config"} -> {"AATL::News::post_count"}, 1) };
